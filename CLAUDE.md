...
- `Editor.Api/` — backend ASP.NET Core host: thin minimal-API endpoints (delegate to services), HTTP middleware, Identity/cookie wiring, validation/policy endpoint extensions, `InfrastructureJsonSerializerContext` (for its own DTOs), `Program.cs` composition root.
- `Editor.Services/` — business layer (references only `Editor.Domain`, no ASP.NET Core): services (`EditingService`, `RegistryService`, `ParadigmService`, `UserService`, `AuthService`, `GrammarDb`, `AwsFilesCache`, `EmailService`, `TurnstileService`), view models + FluentValidation validators (feature folders: `Editing/`, `Registry/`, `Grammar/`, `Users/`, `Auth/`), `ServicesJsonSerializerContext` (the JSON contract for every DTO declared here), document converters (`Converters/`), S3 corpus storage + `VertiIO` (`Corpus/`), and the `BusinessException` hierarchy (`Exceptions/`).
...
- `Turnstile:SecretKey`, `Sentry:Dsn` (Sentry disabled in Development).
...
- `Program.cs` wires everything by hand (no controllers): slim WebApplication builder, manual DI registration, ASP.NET Core Identity Core with cookie auth, and the System.Text.Json source-generated serializer contexts fed into `ConfigureHttpJsonOptions` — a DTO's context lives with the DTO: `InfrastructureJsonSerializerContext` (`Editor.Api`, for the DTOs declared there: `ErrorResponse`, `FrontendConfigResponse`), `ServicesJsonSerializerContext` (`Editor.Services`, all Editing/Registry/Auth/Users/Grammar DTOs) and `VertiJsonSerializerContext` (`Editor.Services/Corpus`, also used by `VertiIO` for the on-disk format). Outbound HTTP clients have their own internal contexts, not registered in the HTTP chain (`EmailJsonSerializerContext`, `TurnstileJsonSerializerContext`). Add a new DTO's `[JsonSerializable]` to the context in its own project.
