...
- **`app/docs/store.ts`** (`useDocumentStore`) — document data, the documents list, pagination (pages of 20 paragraphs via infinite scroll), and **structural editing** with undo/redo. Editing model: `originalDocumentData` is the saved baseline; edits go through `StructureEditor` (`app/docs/structureEditor.ts`, pure functions taking and returning a `DocumentData`) applied via the private `_edit(editor, replaceHistory?)` into a `history`/`historyIndex` stack (`_edit` skips the push when the editor returns the same reference unchanged — several `StructureEditor` operations no-op that way). History exists only for structure editing: the Toolbar's undo/redo is the only history UI, and entering structure-editing mode resets it via `startEditing()`. Per-word markup saves directly against the server and does not go through history. On save, `calculateOperations(original, current)` diffs paragraphs by `concurrencyStamp` into Create/Update/Delete `ParagraphOperation`s sent to `/edit`. Per-word markup goes through `updateSentenceItem(word, patch)`, which **copies the paragraph → sentence → item path**; never mutate document state in place, or the rollback-on-failed-save in `wordEditing.ts` silently stops working (and `React.memo` on `Paragraph`/`Sentence` stops shielding untouched paragraphs from re-rendering). Two invariants around that diff: **every snapshot must describe the same loaded window** — `documentData`, `originalDocumentData` and each `history` entry — so a scroll-in page is appended to all three, otherwise the stamp diff invents `Create`s for already-persisted paragraphs (and `Delete`s after an undo); and structure saving uses its own `saving`/`actionError` state, never the page-level `loading`/`error`, which are reserved for the _initial_ load and replace the whole page.
- **`app/docs/uiStore.ts`** (`useUIStore`) — transient editor UI: `selectedWord`, `displayMode` (`persist`ed to `localStorage` key `editor-ui-store`), the per-field saving flags (single source — components read them, `useWordEditing` writes them; the comment field's saving state lives in `useDebouncedSave` instead, keyed to the word it belongs to), `pendingSaves`, structure-editing mode toggle.
...
- `wordEditing.ts` — plain functions (not a class) for the per-word markup flow: optimistic local update, PUT, then `resolvedOn` on success or rollback on failure. `findNextUnresolvedWord` walks the document once and never returns the word passed in as current; `selectWord` reads both stores through `getState()`, so callers get it without subscribing.
- **Subscriptions in the document tree.** `selectedWord`/`pendingSaves` are _not_ threaded down as props: `LinguisticItem` subscribes to its own word with boolean selectors, and `Paragraph`/`Sentence` are `React.memo`'d, so one word save re-renders only its own paragraph instead of the whole document. Keep store reads there as narrow selectors (`useDocumentStore(s => s.deleteItem)`) — an unfiltered `useStore()` destructure in any of these components re-subscribes the whole subtree to every store update and undoes it.
- `hooks/useDebouncedSave.ts` — the comment autosave. The pending save captures the word key **and** the `onSave` handler at keystroke time, and a change of `resetKey` flushes it first, so text typed for one word can never be PUT onto the next one.
