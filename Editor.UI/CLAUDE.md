...
- `wordEditing.ts` — plain functions (not a class) for the per-word markup flow: optimistic local update, PUT, then `resolvedOn` on success or rollback on failure. `findNextUnresolvedWord` walks the document once and never returns the word passed in as current.
