...
public class GrammarDbTests
{
    ...
    public async Task LookupWords_MatchesLookupWord_PerWord_AndBatchesInOneCall()
    {
        ...
        var batch = await db.LookupWords(["кот", "ката", "невядома"]);

        // Пакетны вынік супадае з паасобным для кожнага слова
        Assert.Equal(await new GrammarDb(Repo()).LookupWord("кот"), batch["кот"]);
    ...
    public async Task LookupWords_DedupsRepeatedWords()
    {
        ...
        var batch = await db.LookupWords(["кот", "кот", "кот"]);
    ...
    private sealed class FakeGrammarRepository(Dictionary<string, IReadOnlyList<FormMatch>> data) : IGrammarRepository
    {
        ...
        public Task<IReadOnlyDictionary<string, IReadOnlyList<FormMatch>>> LookupByNormalizedForms(IReadOnlyCollection<string> normalizedForms, CancellationToken cancellationToken = default)
        {
